...
export const demander = async (req, res) => {
  try {
    ...
    if (!utilisateur) return res.status(404).json({ success: false, message: 'Email non trouve' });
...
export const verifier = async (req, res) => {
  try {
    ...
    if (!reset) return res.status(400).json({ success: false, message: 'Token invalide ou expire' });
