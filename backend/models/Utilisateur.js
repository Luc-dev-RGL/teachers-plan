...
export const UtilisateurModel = {
  ...
  findAll: async (filters = {}) => {
    ...
    if (filters.search) { params.push(`%${filters.search}%`); conditions.push(`(u.email ILIKE $${params.length} OR e.nom ILIKE $${params.length})`); }
  ...
  update: async (id, data) => {
    ...
    params.push(id);
    const { rows } = await query(
      `UPDATE utilisateurs SET ${fields.join(', ')} WHERE id = $${idx} RETURNING id, email, role, actif`,
      params
    );
    return rows[0];
  },
